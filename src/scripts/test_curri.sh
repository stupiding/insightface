...
python -u train_softmax.py --lr 0.1 --fc7-lr-mult 1 --fc7-wd-mult 10 --lr-steps "$LRSTEPS" --data-dir $DATA_DIR --network "$NETWORK" --width-mult 1 --loss-type 0 --margin-m 0.5 --prefix "$PREFIX" --per-batch-size 32 --verbose 20000 --ckpt 2 --max-steps  100001 --pretrained "$PREFIX,0" --version-bn "bn" --version-output "E" #--parallel #> "$LOGFILE" 2>&1 &
LOGFILE="$MODELDIR/log_arcface0.35"
#python -u train_softmax.py --lr 0.1 --fc7-lr-mult 1 --fc7-wd-mult 10 --lr-steps "$LRSTEPS" --data-dir $DATA_DIR --network "$NETWORK" --width-mult 1 --loss-type 4 --margin-m 0.35 --prefix "$PREFIX" --per-batch-size 32 --verbose 20000 --ckpt 2 --max-steps  200001 --pretrained "$PREFIX,5" --version-output "E"  > "$LOGFILE" 2>&1 &
LOGFILE="$MODELDIR/log_arcface0.5"
#LRSTEPS='400000,600000'
#python -u train_softmax.py --lr 0.01 --fc7-lr-mult 1 --fc7-wd-mult 10 --lr-steps "$LRSTEPS" --data-dir $DATA_DIR --network "$NETWORK" --width-mult 1 --loss-type 4 --margin-m 0.5 --prefix "$PREFIX" --per-batch-size 32 --verbose 20000 --ckpt 2 --max-steps  800001 --pretrained "$PREFIX,10" --version-output "E"  > "$LOGFILE" 2>&1 &
