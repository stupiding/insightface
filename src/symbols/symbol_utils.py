...
bn_mom = 0.9
def Linear(data, num_filter=1, kernel=(1, 1), stride=(1, 1), pad=(0, 0), num_group=1, name=None, suffix='', use_global_stats=False):
    conv = mx.sym.Convolution(data=data, num_filter=num_filter, kernel=kernel, num_group=num_group, stride=stride, pad=pad, no_bias=True, name='%s%s_conv2d' %(name, suffix))
    bn = mx.sym.BatchNorm(data=conv, name='%s%s_batchnorm' %(name, suffix), fix_gamma=False,momentum=bn_mom, use_global_stats = use_global_stats)    
    return bn
...
def get_fc1(last_conv, num_classes, fc_type, **kwargs):
  ...
  body = last_conv
  if fc_type=='Z':
    body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn1', use_global_stats = use_global_stats)
    ...
    fc1 = mx.sym.BatchNorm(data=fc1, fix_gamma=True, eps=2e-5, momentum=bn_mom, name='fc1', use_global_stats = use_global_stats)
  elif fc_type=='Ep':
    body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn1', use_global_stats = use_global_stats)
    body = body * (1 / (1 - 0.4))
    fc1 = mx.sym.FullyConnected(data=body, num_hidden=num_classes, name='pre_fc1')
    ...
    bn1 = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn1', use_global_stats = use_global_stats)
    ...
    if num_classes>filters_in:
      ...
      body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=0.9, name='convx_bn', use_global_stats = use_global_stats)
      body = Act(data=body, act_type='relu', name='convx_relu')
      filters_in = num_classes
    else:
      body = last_conv
    body = mx.sym.BatchNorm(data=body, fix_gamma=True, eps=2e-5, momentum=0.9, name='bn6f', use_global_stats = use_global_stats)  
    ...
    if num_classes<filters_in:
      fc1 = mx.sym.BatchNorm(data=fc1, fix_gamma=True, eps=2e-5, momentum=0.9, name='bn6w', use_global_stats = use_global_stats)
      fc1 = mx.sym.FullyConnected(data=fc1, num_hidden=num_classes, name='pre_fc1')
    else:
      fc1 = mx.sym.Flatten(data=fc1)
    fc1 = mx.sym.BatchNorm(data=fc1, fix_gamma=True, eps=2e-5, momentum=0.9, name='fc1', use_global_stats = use_global_stats)
  elif fc_type=="GDC": #mobilefacenet_v1
    conv_6_dw = Linear(last_conv, num_filter=512, num_group=512, kernel=(7,7), pad=(0, 0), stride=(1, 1), name="conv_6dw7_7", use_global_stats = use_global_stats)  
    conv_6_f = mx.sym.FullyConnected(data=conv_6_dw, num_hidden=num_classes, name='pre_fc1')
    fc1 = mx.sym.BatchNorm(data=conv_6_f, fix_gamma=True, eps=2e-5, momentum=bn_mom, name='fc1', use_global_stats = use_global_stats)
  elif fc_type=="GSC": #mobilefacenet_v1
    conv_6_pr = mx.sym.Reshape(data = last_conv, target_shape = (64, 1, 7 *512, 7))
    conv_6_dw = Linear(conv_6_pr, num_filter=1, num_group=1, kernel=(7,7), pad=(0, 0), stride=(7, 7), name="conv_6dw7_7", use_global_stats = use_global_stats)
...
def get_head(data, version_input, num_filter, use_global_stats = False):
    bn_mom = 0.9
    workspace = 256
