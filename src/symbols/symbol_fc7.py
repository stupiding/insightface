...
def Softmax(embedding, gt_label, classes_each_ctx, name, args, cvd=None):
  if cvd is None:
    _weight = mx.symbol.Variable(name + "_weight", shape=(classes_each_ctx, args.emb_size))
    if args.fc7_no_bias:
...
def CosFace(embedding, gt_label, classes_each_ctx, args, cvd=None):
  ...
  s_m = s*m
  gt_one_hot = mx.sym.one_hot(gt_label, depth = classes_each_ctx, on_value = s_m, off_value = 0.0)
  fc7 = fc7-gt_one_hot
  return fc7
...
def ArcFace(embedding, gt_label, classes_each_ctx, name, args, cvd=None):
  ...
  gt_one_hot = mx.sym.one_hot(gt_label, depth = classes_each_ctx, on_value = 1.0, off_value = 0.0)
