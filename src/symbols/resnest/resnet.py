...
class Bottleneck(HybridBlock):
    ...
    def __init__(self, channels, cardinality=1, bottleneck_width=64, strides=1, dilation=1,
                 ...
                 split_drop_ratio=0, act_type='relu', **kwargs):
        ...
        if act_type == 'prelu':
          ...
          self.relu1 = nn.Activation(act_type)
        if self.use_splat:
            ...
            if act_type == 'prelu':
              ...
              self.relu2 = nn.Activation(act_type)
        ...
        if act_type == 'prelu':
          ...
          self.relu3 = nn.Activation(act_type)
...
class ResNet(HybridBlock):
    ...
    def __init__(self, block, layers, cardinality=1, bottleneck_width=64,
                 ...
                 use_splat=False, radix=2, avd=False, avd_first=False, split_drop_ratio=0, **kwargs):
        ...
        with self.name_scope():
            if not deep_stem:
                self.conv1 = nn.Conv2D(channels=64, kernel_size=7, strides=first_stride,
                                       padding=3, use_bias=False, in_channels=3)
            else:
                self.conv1 = nn.HybridSequential(prefix='conv1')
                self.conv1.add(nn.Conv2D(channels=stem_width, kernel_size=3, strides=first_stride,
                                         padding=1, use_bias=False, in_channels=3))
                self.conv1.add(norm_layer(in_channels=stem_width, **norm_kwargs))
                self.conv1.add(nn.Activation(self.act_type) if self.act_type != 'prelu' else nn.PReLU())
        ...
        with layers.name_scope():
            if dilation in (1, 2):
                layers.add(block(planes, cardinality=self.cardinality,
                                 ...
                                 split_drop_ratio=self.split_drop_ratio, act_type=self.act_type))
            elif dilation == 4:
                layers.add(block(planes, cardinality=self.cardinality,
                                 bottleneck_width=self.bottleneck_width,
                                 strides=strides, dilation=pre_dilation,
