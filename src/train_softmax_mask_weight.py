...
class LossValueMetric(mx.metric.EvalMetric):
  ...
  def update(self, labels, preds):
    if 'cls' in self.loss_name:
      ...
      pred = preds[self.loss_idx].asnumpy()
      label = preds[self.loss_idx - 2].asnumpy()
      self.sum_metric += np.sum((pred - label) ** 2) / 2
...
def get_symbol(args, arg_params, aux_params):
  ...
  def get_mask(data):
    ...
    mask_fc3 = mx.sym.FullyConnected(data=mask_fc2, num_hidden=512, name='mask_fc3') + 1.1
    mask_fc3_d = mx.sym.BlockGrad(mask_fc3)

    mask_weight = mx.sym.Activation(data=mask_fc3, act_type='sigmoid')
    mask_weight = mx.sym.where(mask_fc3 < 0, mask_weight, mask_fc3)
    mask_weight = mx.sym.where(mask_fc3 < 1, mask_weight, mask_fc3 / mask_fc3_d)
    return mask_weight

  bn1 = mx.sym.Concat(bn1_orig, bn1_mask, dim=0, name='bn1_concat')
  #embedding = mx.sym.Concat(embedding_orig, embedding_mask ,axis=0, 'fc1_concat')

  mask_weight = get_mask(bn1)
  W, W_m = mx.sym.split(data=mask_weight, axis=0, num_outputs=2)
  #W, W_m = mask_split[0], mask_split[1]

  feats, feats_m = embedding_orig, embedding_mask

  Mask_0 = (mx.sym.sign(feats) != mx.sym.sign(feats_m))
  Mask_1 = (mx.sym.sign(feats) == mx.sym.sign(feats_m))
  
  label_1 = mx.sym.ones_like(W)
  loss_1 = mx.sym.LinearRegressionOutput(data=W, label=label_1, name="loss_1")

  label_0 = mx.sym.zeros_like(W)
  diff = (feats - feats_m) * W_m * Mask_0
  loss_0 = mx.sym.LinearRegressionOutput(data=diff, label=label_0, name="loss_0")
  #loss_0_clip = loss_0 * mx.sym.BlockGrad(loss_0) * 0.35
  #loss_0 = mx.sym.where(loss_0 < loss_0_clip, loss_0, loss_0_clip)
  ...
  if args.loss_type==4:
    fc7 = ArcFace(embedding_mask, gt_label, name, args, cvd)
  elif args.loss_type==5:
    fc7 = CombineFace(embedding_mask, gt_label, name, args, cvd)

  softmax = mx.symbol.SoftmaxOutput(data=fc7, label = gt_label, name='softmax', normalization='valid', use_ignore=True)

  out_list = [mx.symbol.BlockGrad(embedding_orig), mx.sym.BlockGrad(label_0), mx.sym.BlockGrad(label_1)]
  out_list += [loss_0, loss_1, softmax]
...
def train_net(args):
    ...
    for loss_name, loss_idx in zip(['loss_0', 'loss_1', 'loss_cls0'], [3, 4, 5]):
