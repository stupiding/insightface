...
def get_symbol(network, num_layers, args, arg_params, aux_params):
  ...
  num_classes_sum = sum(args.num_classes)
  
  cvd, name = None, 'fc7'
  classes_each_ctx = num_classes_sum
  print('use Parallel or not: {}'.format(args.parallel))

  if args.parallel:
    name += '_sub%d'
    cvd = os.environ['CUDA_VISIBLE_DEVICES'].strip().split(',')
    classes_each_ctx = (num_classes_sum + len(cvd) - 1) // len(cvd)
  ...
  gt_label = all_label[0]
  for i in range(1, len(args.num_classes)):
    start = sum(args.num_classes[:i])
    gt_label = mx.sym.where(all_label[i] >= 0, all_label[i] + start, gt_label)
               
  gt_label = gt_label.reshape([-1, ])

  if args.loss_type in losses:
  ...
  for i in range(len(args.num_classes)):
    start = sum(args.num_classes[:i])
    end = sum(args.num_classes[:i+1])
    print('start, end', start, end)

    gt_label = all_label[i].reshape([-1, ])
    sub_fc7 = mx.sym.slice(data=fc7, begin=(None, start), end=(None, end))
    softmax = mx.symbol.SoftmaxOutput(data=sub_fc7, label = gt_label, name='softmax_%d' % i, normalization='valid', use_ignore=True)
